...
namespace ClipShare_Youtube_.Controllers
{
    ...
    public class ChannelController : Controller
    {
        ...
        public async Task<IActionResult> Create(Channel_vm channel)
        {
            ...
            {
                string res = await DBExcueteCommand.DbInstance.Pr_InsUp_Channel(channel.Name, channel.About, SD.UserId, "0");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> EditChannel(Channel_vm channel)
        {
            if (ModelState.IsValid)
            {
                string res = await DBExcueteCommand.DbInstance.Pr_InsUp_Channel(channel.Name, channel.About, SD.UserId, channel.ChannelId);

                if (res == "-1")
                    ModelState.AddModelError("Edit", "User Is Not Premision For Edit Channel.");
                else if (res == "0" || res == "")
                    ModelState.AddModelError("Edit", "Edit Channel Error.");
            }
